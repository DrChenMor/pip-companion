...
function cannedMessage(data, mood) {
  ...
  const pool = {
    ...
    focused: [
      `${a} readers, ${Math.floor(avg/60)}m ${avg%60}s avg - this is real engagement ♡`,
      `deep readers spending ${Math.floor(avg/60)} minutes each. your storytelling works ✿`,
      `${a} folks reading slow and finishing posts. that's the dream.`,
      `${Math.floor(avg/60)}m+ sessions means they're reading every word. proud of you ♡`,
      `quality readers from ${data.country}. ${Math.floor(avg/60)}m sessions don't lie ✿`,
      `your personal narrative style is landing - ${a} readers staying ${Math.floor(avg/60)}m+`,
      `this is what good content looks like - ${a} engaged readers ♡`,
    ],
    curious: [
    ...
    anxious: [
      `${a} landing but leaving fast (${avg}s avg) - maybe the title promises more than the intro delivers?`,
      `quick visits today - ${avg}s sessions on average. consider a stronger opening line ♡`,
      `${bp}% bounce with short ${avg}s sessions - check if the page loads quickly on mobile`,
      `readers arriving but not staying. try linking related posts in the intro ✿`,
    ],
    proud: [
